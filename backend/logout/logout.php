...
function signOut($sessionID, $email, $userID) {
    echo "Attempting to sign out: Email=$email, SessionID=$sessionID, UserID=$userID"; // Debugging line

    $conn = getDbConnection();
    // Corrected the order of parameters in the SQL query and bind_param call
    $stmt = $conn->prepare("SELECT * FROM sessions WHERE email = ? AND sessionID = ? AND userID = ?");
    $stmt->bind_param("ssi", $email, $sessionID, $userID); // Correct order: email, sessionID, userID
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $deleteStmt->bind_param("ssi", $email, $sessionID, $userID); // Correct order: email, sessionID, userID
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($data['sessionID']) && isset($data['email']) && isset($data['userID'])) {
        signOut($data['sessionID'], $data['email'], $data['userID']);
    } else {
